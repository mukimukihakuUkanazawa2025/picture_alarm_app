...
struct AlermView: View {
    ...
    init() {
        ...
        _wakeUpTime = State(initialValue: calendar.date(bySettingHour: 7, minute: 0, second: 0, of: Date()) ?? Date())
        _leaveTime = State(initialValue: calendar.date(bySettingHour: 8, minute: 0, second: 0, of: Date()) ?? Date())
    }
                ...
                VStack(spacing: 28) {
                    VStack(alignment: .leading, spacing: 12) {
                        Text("起床時間")
                            .font(.headline)
                            .foregroundColor(.white)
                        Text(timeString(from: wakeUpTime))
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        Text("出発時間")
                            .font(.headline)
                            .foregroundColor(.white)
                        Text(timeString(from: leaveTime))
                            .font(.system(size: 48, weight: .bold, design: .rounded))
                            .foregroundColor(.white)
                    }
                    .frame(maxWidth: .infinity, alignment: .leading)
